...
def get_city_coordinates(city: str) -> dict:
    ...
    st.error("Erro: Não foi possível obter as coordenadas da cidade.")
    return None
...
def get_current_weather(lat: float, lon: float) -> dict:
    ...
    st.error("Erro: Não foi possível buscar o clima atual.")
    return None
...
if cidade:
    coords = get_city_coordinates(cidade)
    if coords:
        clima_atual = get_current_weather(coords["lat"], coords["lon"])
        if clima_atual:
            st.subheader(f"🌡️ Clima Atual em {cidade.capitalize()}")
            col1, col2 = st.columns([1, 4])
            with col1:
                st.image(get_icon_url(clima_atual["icone"]), width=80)
            with col2:
                st.markdown(f"**{clima_atual['temperatura']}°C**")
                st.caption(clima_atual["descricao"])
